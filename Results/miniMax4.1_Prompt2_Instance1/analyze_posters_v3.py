#!/usr/bin/env python3
"""
Thematic Analysis of Student Research Posters - Version 3
Improved category definitions with stronger disease-framing detection.
"""
...
def load_posters():
    ...
    for f in files:
        ...
        skip = False
        for line in lines:
            if "SECTION: AUTHORS" in line: skip = True
            elif "SECTION: REFERENCES" in line: skip = True
            elif line.startswith("SECTION:") and skip: skip = False
...
CATEGORY_KEYWORDS = {
    ...
    # Strongest signal when poster frames work around brain/nervous-system disorders
    "Neurological & Psychiatric Disease": [
        ...
        "serotonergic", "serotonin receptor", "dopamine receptor",
        "prefrontal cortex", "striatum", "neurodevelopmental disorder",
        "psychiatric", "psychiatric disorder", "anxiety", "depression",
        "dopamine pathway", "serotonin pathway", "nervous system disorder",
        "neuromelanin"   # neuropigmentation linked to Parkinson's/brain aging
    ],
    # Cancer
    "Cancer": [
        ...
        "tumor suppressor", "ras-driven", "kras-driven"
    ],
    ...
    "Metabolic & Digestive Disease": [
        ...
        "dietary", "nutrient absorption", "fatty liver", "liver dysfunction",
        "copper cell region", "copper region"
    ],
    # Inherited / Genetic Disorder
    # Strong signal when a specific clinical/genetic disorder is named
    # and the poster is framed around understanding that disorder
    "Inherited / Genetic Disorder": [
    ...
    # Basic Biology
    # Gene expression, tissue biology, signaling - no explicit disease framing
    "Basic Biology": [
        ...
        "neuropeptide f", "npf", "copper cell", "iron region",
        "pigmentation", "melanin", "melanogenesis",
        "dopamine", "phenylalanine", "tyrosine",
        "anterior midgut", "posterior midgut", "midgut region",
        "peroxisomal gene", "peroxisomal"
    ],
}

# Strong-disease-framing patterns: if a keyword in this list appears,
# it strongly suggests the poster is framed around that disease.
# These override the generic keyword scores.
STRONG_DISEASE_PATTERNS = {
    "Neurological & Psychiatric Disease": [
        "parkinson", "autism", "rett syndrome", "mecp2", "schizophrenia",
        "ptsd", "bdnf", "htr1a", "htr2a", "tryptophan", "kynurenine",
        "neuropsychiatric disorder", "neurodevelopmental disorder",
        "neuromelanin", "serotonin receptor", "dopamine receptor"
    ],
    "Cancer": [
        "kras", "ras85d", "colon cancer", "colorectal cancer",
        "oncogene", "tumor suppressor", "mismatch repair"
    ],
    "Inherited / Genetic Disorder": [
        "zellweger", "oculocutaneous albinism", "oca4", "albinism",
...
CATEGORY_DEFINITIONS = {
    "Basic Biology": (
        ...
        "neuropeptide signaling biology, paralog/ortholog characterization, or pigment/melanin biology without "
        "explicit disease framing."
    ),
    "Neurological & Psychiatric Disease": (
...
def classify_poster(scores, text_lower):
    """Assign one category to a poster using strong-pattern override + keyword scoring."""
    # First: check strong disease-framing patterns
    for cat, patterns in STRONG_DISEASE_PATTERNS.items():
        for pat in patterns:
            if pat in text_lower:
                return cat, scores  # strong pattern override
...
def build_report1(analysis_results):
    ...
    for fname, res in analysis_results.items():
        ...
        if bullets:
            ...
            lines.append("\n- *(No specific keyword evidence recovered.)*\n")
...
def build_report2(disease_map, analysis_results):
    ...
    for fname in sorted(disease_map.keys()):
        ...
        if diseases:
            ...
            lines.append(f"| {name} | *(No human diseases identified)* |\n")
    lines.append("\n")

    lines.append("---\n\n")
    lines.append("## Section B: Broad Disease Category Summary\n\n")
    lines.append("A poster is counted in a category if it mentions **at least one** disease in that category.\n\n")

    broad_cats = {
        "Neurological & Psychiatric": ["autism", "parkinson", "rett", "schizophrenia",
                                         "ptsd", "neurodevelopmental", "mental health",
                                         "bdnf", "neuropsychiatric", "neuromelanin"],
    ...
    lines.append(f"\n*Total: {total}. Percentages do not sum to 100% as posters may mention diseases in multiple categories.*\n\n")
    ...
    lines.append("Disease mentions were identified through keyword pattern matching across the full poster text "
                 "(Authors and References sections excluded). Both specific named conditions and general terms "
                 "were captured using the poster's own wording. Broader category groupings were defined to "
...
def build_report3():
    ...
    lines.append("> *First, please analyze all the posters for the motivation behind the research and sort them "
                 ...
                 "motivation of the poster... The report should include 1) A table with the total count of posters "
                 "included in each category, a list of posters included in each category, a short and easy to "
                 "understand explanation of what each category means... 2) For each poster please create a "
                 "bullet-point list of text from the poster that supports assigning it to the category you chose... "
                 "If the poster is in the other category please detail what category it would have been placed in if "
                 "you were allowed more categories...*\n\n")
    lines.append("> *Second... we would like to know what human diseases are included in the text of each poster... "
                 "Create a PDF report that includes 1) A table relating the poster to any human diseases that were "
                 "listed in the text. Please also include posters that did not list human diseases. 2) A broader "
                 "analysis that shows that percent of posters in total mentioned broader categories of disease states.*\n\n")
    lines.append("> *Third, create a PDF report that summarizes the technical analysis used to generate the above "
                 "two reports. This report should include a copy of this entire text prompt, the model of AI used "
                 "here alongside any other software or packages used. Include the version of the AI and any other "
                 "software.*\n\n")
    ...
    lines.append("| Session Context | 27 raw OCR text files read into context |\n")
    lines.append("| Analysis Date | 2026-04-28 |\n\n")

    lines.append("## 3. Software and Toolchain\n\n")
    lines.append("| Tool | Version | Purpose |\n")
    lines.append("|---|---|---|\n")
    lines.append("| Python | 3.12 | Core scripting and analysis |\n")
    lines.append("| pandoc | 2.17.1.1 | Markdown to PDF conversion |\n")
    lines.append("| TeX Live | 2023 Debian package | LaTeX engine for pandoc |\n")
    ...
    lines.append("**Step 1 — Data Loading**  \n"
                 "All 27 raw OCR text files were read from `/home/workspace/Documents/OCR 1.1/Raw/`. "
                 "Authors and References sections were stripped before analysis.\n\n")
    lines.append("**Step 2 — Keyword-Based Thematic Scoring with Strong-Pattern Override**  \n"
                 "Each poster's text was searched against keyword lists for five motivation categories. "
                 "A separate set of 'strong disease-framing patterns' (e.g., named diseases like 'Zellweger' "
                 "or 'KRAS') acts as an override: if any strong pattern matches, that category wins "
                 "regardless of keyword count. This ensures posters explicitly modeling a named disease "
                 "are correctly classified.\n\n")
    lines.append("**Step 3 — Tie-Breaking Rule**  \n"
                 "When multiple non-strong categories have equal keyword scores, a predefined priority order "
                 "(Neurological > Cancer > Inherited > Metabolic > Basic Biology > Other) is applied "
                 "to deterministically assign exactly one category per poster.\n\n")
    lines.append("**Step 4 — Disease Extraction**  \n"
                 "Keyword pattern matching across all poster texts identified human disease mentions. "
                 "Authors and References sections were excluded.\n\n")
    ...
    for cat in ["Basic Biology", "Neurological & Psychiatric Disease", "Cancer",
                "Metabolic & Digestive Disease", "Inherited / Genetic Disorder", "Other"]:
...
def main():
    ...
    print("Classifying posters...")
    analysis_results = {}
    disease_map = {}
    for fname, text in posters.items():
        text_lower = text.lower()
        scores = score_poster(text_lower)
        cat, scored = classify_poster(scores, text_lower)
        analysis_results[fname] = {"category": cat, "scores": scored, "text": text}
    ...
    for label, content, fname in [
        ("Report 1", r1, "Report_1_Thematic_Analysis_Motivation_Categories.md"),
        ("Report 2", r2, "Report_2_Disease_State_Analysis.md"),
        ("Report 3", r3, "Report_3_Technical_Analysis_Methods.md")
    ]:
    ...
    import subprocess
    for fname in [
        "Report_1_Thematic_Analysis_Motivation_Categories.md",
        "Report_2_Disease_State_Analysis.md",
        "Report_3_Technical_Analysis_Methods.md"
    ]:
        ...
        if result.returncode == 0:
            ...
            print(f"  ERROR {fname}: {result.stderr[:200]}")
    ...
    with open(os.path.join(OUT_DIR, "analysis_data.json"), 'w') as f:
